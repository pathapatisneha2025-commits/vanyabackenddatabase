  ...
  async (req, res) => {
    try {
      ...
      const result = await pool.query(
        `INSERT INTO products (name, category, price, oldprice, discount, stock, main_image, thumbnails, created_at)
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      const result = await pool.query(
        `UPDATE products
         SET name=$1, category=$2, price=$3, oldprice=$4, discount=$5, stock=$6, main_image=$7, thumbnails=$8, updated_at=CURRENT_TIMESTAMP
         WHERE id=$9
         RETURNING *`,
        [
          name,
